...
class KiteApp:
    ...
    def ltp(self, instruments):
        ltpInfo = self.session.get(f"{self.root_url}/quote/ltp", params={"i": instruments}, headers=self.headers)
        if ltpInfo.status_code==200:
            data = ltpInfo.json()["data"]
            return data
        else:
            return ltpInfo.text
    ...
    def historical_data(self, instrument_token, from_date, to_date, interval, continuous=False, oi=False):
        params = {"from": from_date,
                  ...
                  "oi": 1 if oi else 0}
        lst = self.session.get(f"{self.root_url}/instruments/historical/{instrument_token}/{interval}", params=params,
    ...
    def orders(self, order_id):
        orders = self.session.get(f"{self.root_url}/orders/{order_id}", headers=self.headers).json()["data"]
        return orders
    def get_profile(self):
        profile = self.session.get(f"{self.root_url}/user/profile", headers=self.headers)
